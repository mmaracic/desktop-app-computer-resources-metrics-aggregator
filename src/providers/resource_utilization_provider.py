...
class ResourceUtilizationProvider(MetricProvider):
    ...
    def get_metrics(self) -> list[Metric]:
        ...
        metrics.append(Metric(
            ...
            value=cpu_usage,
            alias="Overall CPU utilization percentage across all processors - shows how much CPU capacity is currently being used",
        ...
        metrics.append(Metric(
            ...
            metric_type=MetricType.FLOAT,
            value=mem.percent,
            alias="System RAM utilization percentage - indicates how much of the available memory is currently in use",
        ...
        metrics.append(Metric(
            ...
            metric_type=MetricType.FLOAT,
            value=disk.percent,
            alias="Root filesystem disk usage percentage - shows how much of the system drive is occupied with data and applications",
        ...
        try:
            ...
            if freq and freq.current:
                metrics.append(Metric(
                    ...
                    value=freq.current,
                    alias="Current CPU operating frequency in megahertz - indicates the actual speed at which the processor is running",
                    component_type=ComponentType.CPU
                ))
        ...
        try:
            ...
            if cpu_stats.context_switches_voluntary:
                metrics.append(Metric(
                    ...
                    alias="Total voluntary context switches - counts CPU task switches initiated by the system when processes voluntarily yield control",
                    component_type=ComponentType.CPU
                ))
            if cpu_stats.context_switches_involuntary:
                metrics.append(Metric(
                    ...
                    alias="Total involuntary context switches - counts CPU task switches forced by the system due to resource constraints",
                    component_type=ComponentType.CPU
                ))
        except (OSError, AttributeError) as exc:
        ...
        metrics.append(Metric(
            ...
            alias="Available system memory in megabytes - shows how much RAM is currently free for new applications and processes",
            component_type=ComponentType.RAM
        ))
        ...
        try:
            ...
            if swap.total > 0:
                metrics.append(Metric(
                    ...
                    metric_type=MetricType.FLOAT,
                    value=swap.percent,
                    alias="Swap space utilization percentage - indicates how much of the virtual memory (page file/swap) is currently in use",
                    component_type=ComponentType.RAM
                ))
                metrics.append(Metric(
                    name="swap_used_mb",
                    metric_type=MetricType.INTEGER,
                    value=int(swap.used),
                    alias="Used swap space in megabytes - shows the amount of virtual memory currently being used on disk",
                    component_type=ComponentType.RAM
                ))
        except (OSError, AttributeError) as exc:
        ...
        try:
            ...
            metrics.append(Metric(
                ...
                alias="Total network bytes transmitted - cumulative count of data sent out through all network interfaces",
            ...
            metrics.append(Metric(
                ...
                alias="Total network bytes received - cumulative count of data received through all network interfaces",
                component_type=ComponentType.NETWORK
            ))
        except (OSError, AttributeError) as exc:
        ...
        try:
            ...
            if disk_io:
                metrics.append(Metric(
                    ...
                    alias="Total disk read operations - counts the number of times data has been read from the storage device",
                    component_type=ComponentType.DISK
                ))
                metrics.append(Metric(
                    ...
                    alias="Total disk write operations - counts the number of times data has been written to the storage device",
                    component_type=ComponentType.DISK
                ))
        except (OSError, AttributeError) as exc:
